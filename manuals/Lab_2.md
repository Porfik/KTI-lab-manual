...
После перевода SELinux в режим Permissive сайт открывается, значит дело именно в механизме безопасности. Отключение SELinux не является безопасным решением и не является [примером хороших практик](https://stopdisablingselinux.com), гораздо грамотнее написать правила, разрешающие конкретные операции. В нашем случае, чтобы разрешить трафик от NGINX, можно добавить правило:
