...
>[!NOTE]
>Ветки в Git — это независимые линии разработки, позволяющие работать над разными частями проекта параллельно, не друг на друга. Каждая ветка сохраняет свою историю изменений, что даёт возможность изолированно разрабатывать новые функции, исправлять ошибки или экспериментировать, а затем безопасно интегрировать результаты в основную ветку (например, `master`). Это ключевой инструмент для организации командной работы и поддержания стабильности проекта.
>
>Во втором этапе лабораторной работы создаётся базовая HTML-структура страницы в ветке `html-structure`, от которой ответвляются две новые ветки: `add-css` (для стилей) и `add-script` (для JavaScript).** В обеих ветках добавляется один и тот же элемент `<div class="matrix" id="matrixCode"></div>`, после чего ветка `add-script` сливается обратно в `html-structure`, обогащая её интерактивным функционалом. Затем обновлённая `html-structure` вливается в `main`, а вслед за ней — ветка `add-css`, что завершает сборку полной версии страницы без конфликтов благодаря одинаковым промежуточным изменениям.
