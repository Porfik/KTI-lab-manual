...
IP-адрес мы настраивали или получали в процессе установки. Однако он может измениться в зависимости от сети, к которой подключена виртуальная машина. Узнаем IP-адрес сетевого интерфейса ВМ, для этого в введём в консоль виртуальной машины команду:
...
Но при разработке удобнее подключаться к виртуальной машине, используя редактор Visual Studio Code. При помощи него можно удобно редактировать файлы на удаленном сервере, одновременно получая доступ к терминалу и файловой системе, и именно его мы будем использовать дальше в работах.
...
После этого откроется строка для ввода команды подключения, вводим в нее команду с своей учетной записью и IP-адресом ВМ:
...
Перейдём во вкладку **Explorer** и нажмем `Open Folder`

![](../images/lab_1/1.58.png)

В всплывающем оке предложат ввести путь к директории которую вы хотите открыть, по умолчанию стоит путь к домашней директории вашего пользователя, что нам и нужно.
...
И уже в ней файл `authorized_keys` без расширений.
